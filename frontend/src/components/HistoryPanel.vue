<template>
  ...
  >
    <div class="px-4 py-2 border-b flex items-center justify-between shrink-0" style="border-color: var(--border);">
      ...
      <button @click="emit('close')" class="transition-colors" style="color: var(--text-muted);">
    ...
    <div class="flex-1 overflow-y-auto">
      <div v-for="(rev, idx) in historyList" :key="rev.hash" class="p-4 border-b transition-colors group" style="border-color: var(--border);"
        ...
        <div v-if="editingLabelHash === rev.hash" class="flex gap-1 mb-2">
          <input v-model="editingLabelValue" @keydown.enter="saveLabel" @keydown.esc="cancelEditLabel" :placeholder="t.labelPlaceholder" autofocus
            class="flex-1 px-2 py-1 text-xs rounded-md border outline-none" style="background: var(--bg-app); border-color: var(--border); color: var(--text-primary); font-family: inherit;" />
          <button @click="saveLabel" class="px-2 py-1 text-xs rounded-md" style="background: var(--accent); color: white;">OK</button>
          <button @click="cancelEditLabel" class="px-1 py-1 text-xs rounded-md" style="color: var(--text-muted);">✕</button>
        </div>
        ...
        <!-- Add/edit label link -->
        <button v-if="editingLabelHash !== rev.hash && !commitLabels[rev.hash]" @click="startEditLabel(rev.hash)" class="ts-xs mb-2 opacity-0 group-hover:opacity-100 transition-opacity" style="color: var(--text-muted);">+ {{ t.addLabel }}</button>
        ...
        <div v-if="pendingRollbackHash === rev.hash" class="flex gap-2 items-center rounded-lg px-3 py-2 mb-1" style="background: var(--accent-light); border: 1px solid var(--accent);">
          ...
          <button @click="confirmRollback" class="px-2 py-1 text-xs rounded-lg history-revert-btn">{{ t.confirm }}</button>
          <button @click="cancelRollback" class="px-2 py-1 text-xs rounded-lg" style="background: var(--bg-hover); color: var(--text-secondary);">{{ t.libResetModalCancel }}</button>
        </div>
        <div v-else class="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <button @click="requestRollback(rev.hash)" class="flex-1 py-1 text-xs rounded-lg transition-all history-revert-btn">{{ t.revertTo }}</button>
        </div>
      </div>
